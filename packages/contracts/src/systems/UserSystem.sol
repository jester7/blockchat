...
contract UserSystem is System {
    ...
    function setUsername(string memory username) public {
        ...
        string memory userPicture = Users.getUserPicture(sender);
        
        if (bytes(userPicture).length == 0) {
    ...
    function getOrSetToDefaults() public returns (string memory) {
        bytes32 sender = addressToEntity(_msgSender());
        if (bytes(Users.getUsername(sender)).length == 0) {
            Users.set(sender, UsersData({
            ...
            return Users.getUsername(sender);
        }
    }

    function hasUserPicture() public view returns (bool) {
        bytes32 sender = addressToEntity(_msgSender());
        return bytes(Users.getUserPicture(sender)).length > 0;
    }
}
